...
def call_NFW(       
        ...
        verbose):
    '''
    Call NFW_funcs.pyx for multiprocessing purpose
    ...
    combined = np.column_stack((multi_rvals, multi_thetavals, multi_xvals, multi_yvals))
    with open('output.txt', 'a') as f:
        np.savetxt(f, combined, delimiter='\t', fmt='%d')
...
if __name__ == '__main__':
    ...
    for i in trange(0,len(lenses),chunk):
        ...
        multi_offsets=np.delete(multi_offsets, 0,0)
        offset_chunks.append(multi_offsets)
    #%%
    ...
    args = ((np.array(lense_chunks[c]["M_halo"].data).astype('<f8'), 
             np.array(lense_chunks[c]["z"].data).astype('<f8'), 
             multi_mass_per_point, 
             offset_chunks[c], 
             seeds, 
             cdf_resolution, 
             return_xy, 
             verbose) for c in range(len(lense_chunks)))


    
    pool.starmap(call_NFW,args)
